...
- *How much.* A deployment may cap the bytes each org keeps across both buckets, with one cap for free orgs and one for paid (`private.config.free_media_storage_limit_bytes` and `paid_media_storage_limit_bytes`, null and off by default, migration `media_storage_limit`); which applies is decided by `private.org_is_paid`, as for the other plan limits, but unlike them a paid org can be capped too. Policies cannot enforce it: Storage runs them on a trial insert made before the upload, when the size is only what the client declared, and writes the real row, with the measured size, as its own role, which policies do not apply to. A trigger on `storage.objects` runs both times: the first refuses a declared size that does not fit, or anything once the org is full, before the bytes are sent; the second refuses the measured size, and Storage then removes the bytes. Per-org advisory locks keep simultaneous uploads from fitting one by one and overflowing together. The refusal is a 42501 with a message for the person uploading (the free plan's says that upgrading raises it), which Storage passes through and `lib/whiteboard/media.ts` recognises and shows. Settings → General shows the usage against the org's own cap (`org_media_usage`) while there is one.
