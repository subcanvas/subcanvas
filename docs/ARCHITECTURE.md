...
- *How much.* A deployment may cap the bytes each org keeps across both buckets (`private.config.media_storage_limit_bytes`, null and off by default, migration `media_storage_limit`). Policies cannot enforce it: Storage runs them on a trial insert made before the upload, when the size is only what the client declared, and writes the real row, with the measured size, as its own role, which policies do not apply to. A trigger on `storage.objects` runs both times: the first refuses a declared size that does not fit, or anything once the org is full, before the bytes are sent; the second refuses the measured size, and Storage then removes the bytes. Per-org advisory locks keep simultaneous uploads from fitting one by one and overflowing together. The refusal is a 42501 with its own message, which Storage passes through and `lib/whiteboard/media.ts` recognises. Settings → General shows the usage (`org_media_usage`) while a cap is set.
