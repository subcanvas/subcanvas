...
| `GITHUB_TOKEN` | Optional. Server only. Any GitHub token with no scopes at all (a fine-grained token with "Public repositories (read-only)" access is right). "Import from GitHub" reads public repositories and works without it, but GitHub then allows only 60 requests an hour from your server's address, about 20 imports. With a token it is 5,000. It is used for the rate limit only; private repositories are refused either way. |
